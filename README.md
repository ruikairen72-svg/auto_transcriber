...
### ELAN 风格多层时间轴编辑器（Step 4）
- 🎬 **视频同步播放** — 播放头实时跟随，自动滚动保持居中
- ✂️ **拖拽调整** — 拖拽 segment 边缘调整起止时间，拖拽中间移动整体位置
- 🔄 **跨层拖拽** — 将 segment 拖到其他层的行上，自动变更层归属
- 🖊️ **文本编辑** — 点击 segment 后在右侧面板编辑文本、时间和层归属
- ➕ **双击添加** — 双击空白区域快速创建新 segment
- 🗑️ **删除** — 选中 segment 后一键删除
- ↩️ **撤销/重做** — 支持 Ctrl+Z / Ctrl+Shift+Z，最多 50 步历史
- 🔍 **搜索** — 关键字搜索，匹配的 segment 高亮显示
- 👁️ **层管理** — 勾选/取消勾选控制层显示，▶/▼ 展开/折叠子层
- 🌲 **层级缩进** — 父子层级自动缩进显示，清晰展示层级结构
- 🔍 **缩放** — Ctrl+滚轮缩放时间轴
...
| 操作 | 方式 |
|------|------|
| 选择 segment | 点击 segment 色块 |
| 修改文本 | 在右侧面板编辑，修改立即生效 |
| 调整时间 | 拖拽 segment 左右边缘，或在面板中输入 |
| 移动 segment | 拖拽 segment 中间 |
| 变更层归属 | 拖拽 segment 到其他层的行 / 在面板中选择 |
| 添加 segment | 双击时间轴空白区域 |
| 删除 segment | 选中后点击 Delete 按钮 |
| 撤销 | Ctrl+Z / 点击 ↩ 按钮 |
| 重做 | Ctrl+Shift+Z / 点击 ↪ 按钮 |
| 搜索 | 在搜索框中输入关键字 |
| 隐藏/显示层 | 取消/勾选控制面板中的层复选框 |
| 展开/折叠子层 | 点击层名旁的 ▶/▼ 图标 |
| 播放视频 | 点击视频播放按钮，播放头同步移动 |
