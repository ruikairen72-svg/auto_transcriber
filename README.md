...
适用于语言学、教育学、心理学等需要高质量语音转写与标注的场景。
...
#### 📐 面板自适应拖拽
- 视频与工作面板之间、工作面板与时间轴之间有可拖拽的分隔条
- 拖拽即可调整各区域大小比例
- 调整后的布局自动保存到 localStorage，刷新后恢复
...
#### 🔀 拖拽调整层顺序
- 拖拽层名称左侧的手柄（⠿）即可调整层在时间轴中的上下位置
- 新顺序自动保存到 localStorage，刷新后恢复
- 层颜色基于层 ID 哈希分配，不随顺序变化

#### 🎨 层颜色固定
- 每个层基于层 ID 分配固定颜色（20 色调色板）
- 层标签左侧有彩色边框，segment 色块使用相同颜色
- 层顺序调整、展开/折叠后颜色不变，视觉始终保持统一

#### 💾 数据持久化（防止中途数据丢失）

在 Step 4 时间轴编辑器右侧控制面板中，提供完整的数据保存与恢复方案：

- **💾 保存进度**：手动保存当前工作进度，生成 `transcript_work_时间戳.json` 文件（保存在 `saves/` 目录）
- **🔄 自动保存**：每 30 秒自动保存到 `saves/autosave.json`，刷新页面或意外关闭后自动检测
- **📂 加载工作**：选择之前保存的 `*.json` 工作文件，一键恢复所有修改
- **恢复横幅**：进入 Step 4 时自动检测未完成工作，点击即可恢复

**数据完整性**：所有修改（文本、时间、层归属、新增/删除 segment、层可见性、层顺序）均被完整保存和恢复，确保工作进度不丢失。
...
### 高级交互
- **📐 面板自适应拖拽**：视频、工作面板、时间轴之间通过拖拽分隔条自由调整布局，比例自动保存
- **📋 层管理面板**：集中控制所有层的显示/隐藏，按层级树结构呈现，隐藏的层可随时调出
- **🔀 拖拽调整层顺序**：拖拽层名称调整上下次序，灵活组织标注视图
- **🎨 层颜色固定**：每个层与其 segment 色块颜色一致，不受顺序和展开/折叠影响
...
|--------|-------------------|--------------------------|
| 转写效率 | 10分钟视频需 4-6 小时 | 1 小时视频约 120 分钟 |
...
- **保存工作进度**（防止数据丢失）：
  - 随时点击 **“💾 保存进度”** 按钮，保存当前所有修改
  - 系统每 30 秒自动保存一次（`saves/autosave.json`）
  - 刷新页面后，顶部会出现恢复横幅，点击“恢复工作”继续编辑
  - 也可通过 **“📂 加载工作”** 按钮，加载之前保存的工作文件
